...
_GLOBAL_PYGLET_WINDOW_REFCOUNT = 0
...
class SofaEnv(gym.Env, metaclass=abc.ABCMeta):
    ...
    def close(self) -> None:
        ...
        if hasattr(self, "_window") and self._window is not None:
            if self.render_framework == RenderFramework.PYGLET:
                # Use global shared window if available; only close when last user
                global _GLOBAL_PYGLET_WINDOW, _GLOBAL_PYGLET_WINDOW_REFCOUNT
                try:
                    if _GLOBAL_PYGLET_WINDOW is not None and self._window is _GLOBAL_PYGLET_WINDOW:
                        _GLOBAL_PYGLET_WINDOW_REFCOUNT -= 1
                        if _GLOBAL_PYGLET_WINDOW_REFCOUNT <= 0:
                            try:
                                _GLOBAL_PYGLET_WINDOW.close()
                            finally:
                                _GLOBAL_PYGLET_WINDOW = None
                                _GLOBAL_PYGLET_WINDOW_REFCOUNT = 0
                    else:
    ...
    def _init_sim(self) -> None:
        ...
        if not self.internal_render_mode == RenderMode.NONE:
            if not isinstance(self.scene_creation_result, dict) or "camera" not in self.scene_creation_result:
                raise KeyError("When creating a scene that should be rendered, please make sure createScene() returns a dictionary with key 'camera'.")

            camera_obj = self.scene_creation_result["camera"]
            try:
                self._camera_object = camera_obj.sofa_object
            except Exception:
                self._camera_object = camera_obj
    ...
    def _update_rgb_buffer(self) -> np.ndarray:
        ...
        if self.render_framework == RenderFramework.PYGLET and self._window is not None:
            # Keep the window context current and process pending events to avoid stale black frames.
    ...
    def _init_pyglet_window(self):
        ...
        # Create or reuse a global pyglet window for this process. Reusing avoids
        # spawning many windows when multiple environments or scenes are created.
        global _GLOBAL_PYGLET_WINDOW, _GLOBAL_PYGLET_WINDOW_REFCOUNT
        if _GLOBAL_PYGLET_WINDOW is not None:
            # Reuse existing window
            self._window = _GLOBAL_PYGLET_WINDOW
            _GLOBAL_PYGLET_WINDOW_REFCOUNT += 1
        else:
            # Create the pyglet window and register it as global
            ...
            _GLOBAL_PYGLET_WINDOW = self._window
            _GLOBAL_PYGLET_WINDOW_REFCOUNT = 1

        # Mouse interaction for camera control in HUMAN/REMOTE window.
        self._window.push_handlers(
            on_mouse_drag=self._on_pyglet_mouse_drag,
            on_mouse_scroll=self._on_pyglet_mouse_scroll,
        )

        self._init_opengl()

    def _get_camera_vector(self, key: str) -> np.ndarray:
        if hasattr(self._camera_object, "findData"):
            data = self._camera_object.findData(key)
            if data is not None:
                return np.asarray(data.value[:3], dtype=np.float64)

        attr = getattr(self._camera_object, key, None)
        if attr is not None and hasattr(attr, "value"):
            return np.asarray(attr.value[:3], dtype=np.float64)

        raise AttributeError(f"Camera has no data field '{key}'.")

    def _set_camera_vector(self, key: str, value: np.ndarray) -> None:
        value_list = [float(value[0]), float(value[1]), float(value[2])]
        ...
        raise AttributeError(f"Camera has no writable data field '{key}'.")
    ...
    def _rotate_vector(vec: np.ndarray, axis: np.ndarray, angle_rad: float) -> np.ndarray:
        ...
        return vec * c + np.cross(axis_unit, vec) * s + axis_unit * np.dot(axis_unit, vec) * (1.0 - c)

    def _on_pyglet_mouse_drag(self, x, y, dx, dy, buttons, modifiers):
        mouse = self.pyglet.window.mouse

        try:
            cam_pos = self._get_camera_vector("position")
            cam_look = self._get_camera_vector("lookAt")
        except Exception:
            return

        view_vec = cam_look - cam_pos
        ...
        if buttons & mouse.LEFT:
            ...
            yaw = -float(dx) * 0.004
            pitch = -float(dy) * 0.004

            cam_to_pos = cam_pos - cam_look
            cam_to_pos = self._rotate_vector(cam_to_pos, world_up, yaw)

            view_dir = cam_look - (cam_look + cam_to_pos)
            ...
            cam_to_pos = self._rotate_vector(cam_to_pos, right, pitch)
            self._set_camera_vector("position", cam_look + cam_to_pos)

        elif buttons & mouse.RIGHT:
            # Pan camera parallel to image plane.
            ...
            scale = max(view_dist * 0.002, 1e-5)
            delta = (-float(dx) * right + float(dy) * up) * scale

            self._set_camera_vector("position", cam_pos + delta)
            self._set_camera_vector("lookAt", cam_look + delta)

    def _on_pyglet_mouse_scroll(self, x, y, scroll_x, scroll_y):
        try:
            cam_pos = self._get_camera_vector("position")
            cam_look = self._get_camera_vector("lookAt")
        except Exception:
        ...
        cam_to_pos = cam_pos - cam_look
        dist = np.linalg.norm(cam_to_pos)
        if dist < 1e-9:
            return

        # scroll_y > 0: zoom in, scroll_y < 0: zoom out
        zoom_factor = math.exp(-float(scroll_y) * 0.1)
        new_dist = np.clip(dist * zoom_factor, 0.02, 5.0)
        cam_to_pos = cam_to_pos * (new_dist / dist)
    ...
    def _init_opengl(self):
        ...
        self.opengl_gl.glClearColor(0.2, 0.3, 0.4, 1.0)
    ...
    def _update_sofa_visuals(self) -> None:
        ...
        # Update and apply the camera transformation matrix before drawing the scene
