...
class MagController(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, event):
        ...
        for i in range(0, len(self.instrument.index_mag)):
            ...
            # RL/SOFA simulation mode:
            # Use the desired uniform magnetic field directly.
            # This bypasses the Navion/eMNS current inversion and forward model,
            # avoiding NaN when actualPos is outside the eMNS calibration workspace.
            field = np.asarray(self.field_des, dtype=np.float64).reshape(3)
