...
from mcr_sim_ros import mcr_mag_controller_ros
...
class ControllerSofa(Sofa.Core.Controller):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mag_field_init = np.asarray(mag_field_init, dtype=np.float32).copy()

        self.dfield_angle = 0.0

        self.mag_controller = mcr_mag_controller_ros.MagController(
        ...
        self.mag_controller.field_des = self.mag_field_init.copy()
    ...
    def insertRetract(self, val):
        # Keep the ROS version aligned with the currently used closed-loop test setting.
        d_step = 0.0008
    ...
    def reset(self) -> None:
        """Reset magnetic field and insertion state."""
        super().reset()
        self.mag_controller.field_des = self.mag_field_init.copy()
        self.dfield_angle = 0.0
    ...
    def set_mag_field_des(self, B):
        """Set the desired magnetic field directly, e.g. from RL strict sync loop."""
        self.mag_controller.field_des = np.asarray(B, dtype=np.float32).copy()
