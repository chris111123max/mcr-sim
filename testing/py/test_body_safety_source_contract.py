"""CPU-only wiring checks for the shared Reward V9 curve/safety state."""
...
class BodySafetySourceContractTest(unittest.TestCase):
    def test_all_algorithms_share_the_78_dimensional_environment_state(self):
        ...
        self.assertIn("bend_severity_features", actor_builder)
    ...
    def test_reward_v9_uses_recoverable_behavior_failures_and_curve_control(self):
        source = (PROJECT_ROOT / "mcr_sim" / "mcr_rl_env.py").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn('"unsafe_curve_insertion_penalty"', source)
