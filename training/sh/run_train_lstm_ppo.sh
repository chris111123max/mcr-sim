...
# ``--nohup`` is launcher-owned.  It creates the final run directory before
# torchrun starts and captures outer HCCL/native output next to rank logs.
MANAGED_NOHUP=0
FORWARD_ARGS=()
for arg in "$@"; do
    if [[ "$arg" == "--nohup" ]]; then
...
if ((MANAGED_NOHUP == 1)) && [[ "${MCR_MANAGED_NOHUP_CHILD:-0}" != "1" ]]; then
    ...
    nohup env \
        ...
        bash "$SCRIPT_DIR/run_train_lstm_ppo.sh" "$@" \
        >"$LAUNCHER_LOG" 2>&1 </dev/null &
    TRAIN_PID=$!
    echo "[STARTED] algorithm=lstm_ppo pid=$TRAIN_PID"
...
TRAIN_SCRIPT="$PYTHON_ROOT/training/py/train_lstm_ppo.py"
...
if ((DISTRIBUTED == 1)) && [[ "${WORLD_SIZE:-1}" -le 1 ]]; then
    ...
    echo "[LAUNCH] LSTM-PPO distributed world_size=$WORLD_SIZE_ARG command=$TORCHRUN_BIN"
