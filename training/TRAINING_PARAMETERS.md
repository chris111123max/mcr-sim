...
Reward profile v9 使用选定中心线上的连续弧长进度 `Phi in [0,1]`，进度项为
...
| 急弯未对准时正向插入 | -0.10 | 正插入动作 × 20 mm弯曲度 × 未来切向未对准度；对准或回撤时为零 |
...
`run_config.json` 会完整保存 Reward v9、实际 observation shape/dtype，`train_summary.csv`
...
前方 5/10/20 mm 的局部切向，并显式提供三档弯曲严重度，使 MLP 也能预见即将到来的弯道。PPO、LSTM-PPO、SAC
...
语义与 Reward V9 均已改变，旧 checkpoint 不能直接续训。
