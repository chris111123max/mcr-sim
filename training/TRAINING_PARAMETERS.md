...
奖励使用实际距离变化，而不是只判断“变近/变远”。Reward profile v3 按每个回合
随机采样后的真实 start-to-target 路线长度归一化：完整路线的正向进度奖励硬封顶
为 `+100`，全部 waypoint 奖励硬封顶为 `+20`。回撤不会恢复已经用掉的正向额度，
因此往返振荡不能刷分；任何终止失败即使发生在目标前也保持负回报。所有单步和
终止项继续控制在 `O(1)..O(100)`，避免 SAC critic/actor 被千级目标推入饱和。

| 奖励项 | 权重 | 含义 |
|---|---:|---|
| waypoint/目标连续接近 | 总额最多 +100 | `100 × Δdistance / 本回合路线长度`；远离对称扣分，正向累计封顶 |
...
`max_grad_norm=0.5`。`run_config.json` 会完整保存 Reward v3，`train_summary.csv`
同时记录奖励分项、正回报失败率、已用进度/waypoint 奖励比例、无进展次数、
