...
2048 是最大步数，不要求每个回合都运行满：到达目标、出血管、错误分支、长期
无进展或出现非有限状态时都会提前结束。

## 奖励

Reward profile v7 使用选定中心线上的连续弧长进度 `Phi in [0,1]`，进度项为
`100 × (Phi_t-Phi_{t-1})`。首次定位允许搜索完整目标路线；之后只在上一进度前后
20/40 mm 内投影，并拒绝超过 2 mm 单步物理进度的候选。空间相邻的 180° 回头弯或
其他分支因此不能制造进度跳变。往返振荡净奖励为 0，必要回退后再次前进会恢复奖励。

| 奖励项 | 权重 | 含义 |
|---|---:|---|
| 连续路线进度 | 净额范围 -100..+100 | `100 × ΔPhi`；远离对称扣分，纠偏后可恢复，振荡净值为零 |
...
| 超时 | -120 | 2048 步仍未完成；步数代价保证最大导航额度下仍为负 |
...
`run_config.json` 会完整保存 Reward v7、实际 observation shape/dtype，`train_summary.csv`
...
使用完全相同的状态。观测维度仍为 78，但导航字段语义已经改变，Reward V7 之前的
checkpoint 不能直接续训。
