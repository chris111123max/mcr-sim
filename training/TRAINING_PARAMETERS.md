...
| 奖励项 | 权重 | 含义 |
|---|---:|---|
| waypoint/目标连续接近 | 5.0 | `5 × Δdistance / 1 mm`，远离时对称扣分 |
| 到达中间 waypoint | +5 | 一次性小奖励，避免约 100 个 waypoint 主导总回报 |
| 最终成功 | +1000 | 进入 3 mm 目标且仍在血管内 |
| 出血管 | -1000 | 安全比达到 1.00 时立即终止 |
| 超时 | -500 | 4096 步仍未完成 |
| 每步代价 | -0.05 | 鼓励更短路径，但不压倒连续进度奖励 |

`out_of_vessel_safety_ratio=1.00` 使用
`径向偏移 / (局部血管半径 − 导管半径)` 判断，因而对应导管外缘到达血管内壁；
不再额外允许 10% 越界容差。
