...
非有限状态作为仿真异常紧急结束。错误分支和长期无进展只扣分，允许策略恢复。

## 奖励

Reward profile v8 使用选定中心线上的连续弧长进度 `Phi in [0,1]`，进度项为
`200 × (Phi_t-Phi_{t-1})`。首次定位允许搜索完整目标路线；之后只在上一进度前后
20/40 mm 内投影，并拒绝超过 2 mm 单步物理进度的候选。空间相邻的 180° 回头弯或
其他分支因此不能制造进度跳变。往返振荡净奖励为 0，必要回退后再次前进会恢复奖励。

| 奖励项 | 权重 | 含义 |
|---|---:|---|
| 连续路线进度 | 净额范围 -200..+200 | `200 × ΔPhi`；远离对称扣分，纠偏后可恢复，振荡净值为零 |
| tip/整段导管持续接近越界 | -0.04 | tip 贴壁与 whole-body 中心距越界阈值 0.5 mm 内取较大风险；安全插入仍为正 |
| tip 穿壁/整段中心越界深度 | -1.00 | tip 穿入深度与 whole-body 中心越过管壁深度取较大归一化值 |
| 偏离目标分支 | -0.20 | 选定路径与完整中心线图的距离差形成连续扣分 |
| 请求回撤 | -0.002 | 按负插入动作幅度轻量扣分，允许长弯道中必要的纠偏 |
| 持续无进展 | 0（仅诊断） | 256 步窗口净接近不足 1 mm 时记录，但不扣分、不终止；由每步代价和超时约束 |
| 进入错误分支 | -1.0/步 | 路径差超过 2 mm 并持续 5 步后逐步扣分，返回目标路线即可恢复 |
| 最终成功 | +300 | 进入 3 mm 目标且导管中心未在内腔外 |
| 出血管 | -210 | 整段导管中心超出 SDF 管壁 0.5 mm，连续 3 步时终止 |
| 非有限状态 | -220 | observation/reward 出现非有限值时紧急终止 |
| 无进展终止 | 关闭 | Reward V8 不因无进展结束回合 |
| 超时 | -200 | 2048 步仍未完成；拿满进度后超时回报仍为负 |
...
`run_config.json` 会完整保存 Reward v8、实际 observation shape/dtype，`train_summary.csv`
同时记录奖励分项、无进展/错误分支行为事件、正回报失败率、终止路线势、
...
相对 tip 的局部坐标及其回正方向。策略还接收前方 10/20 mm 的连续移动引导点和
前方 5/10/20 mm 的局部切向，使 MLP 也能预见即将到来的弯道。PPO、LSTM-PPO、SAC
使用完全相同的状态。所有向量都转换到导管尖端局部坐标系；策略不接收绝对 XYZ，
也不再接收容易记忆训练血管相位的 route completion percentage。对应槽位改为经最长
训练路线归一化的剩余中心线路程，它对整体平移和旋转不变。观测维度仍为 78；该槽位
语义与 Reward V8 均已改变，旧 checkpoint 不能直接续训。
...
1. `simple_fixed`：B01、B02、C01、C02，固定母版几何；
2. `simple_full_dr`：同四条简单血管，启用命令所定义的完整域随机化范围；
3. `all_fixed`：加入 B03–B05、C03–C05，重新关闭域随机化；
4. `all_full_dr`：全部十条血管，重新启用完整域随机化。

每根当前活跃血管保存最近 100 个 episode 的滚动结果；每一根都积累完整 100 个样本、
3 mm 成功率达到 50%，并连续维持 3 个全局 epoch 才会升级。缺少样本或任一血管未达标
都会清零连续计数。阶段升级时所有旧窗口清空，固定几何成绩不能证明随机化任务已掌握，
