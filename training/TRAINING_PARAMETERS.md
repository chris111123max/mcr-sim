...
奖励使用实际距离变化，而不是只判断“变近/变远”。Reward profile v2 将一次最大
插入（0.2 mm）对应为 1 个连续进度单位（并裁剪到 `[-1, 1]`），让正确前进始终
压过近壁塑形，同时把单步和终止目标控制在 `O(1)..O(100)`，避免 SAC 网络饱和。

| 奖励项 | 权重 | 含义 |
|---|---:|---|
| waypoint/目标连续接近 | 1.0 | `Δdistance / 0.2 mm`，远离时对称扣分 |
| 到达中间 waypoint | +5 | 强化可到达的分段导航反馈 |
| tip 持续贴近管壁 | -0.10 | 保留风险提示，但不压过一次正确插入 |
| tip 表面穿壁 | -1.0 | 按 tip SDF 穿入深度/导管半径归一化扣分 |
| 偏离目标分支 | -0.25 | 选定路径与完整中心线图的距离差形成连续扣分 |
| 请求回撤 | -0.05 | 按负插入动作幅度扣分，回撤能力仍保留 |
| 持续无进展 | -0.025 | 256 步窗口净接近不足 1 mm 时逐步启用 |
| 进入错误分支 | -100 | 路径差超过 2 mm 并持续 5 步时终止 |
| 最终成功 | +100 | 进入 3 mm 目标且导管中心未在内腔外 |
| 出血管 | -100 | 整段导管中心超出 SDF 管壁 0.5 mm，连续 3 步时终止 |
| 无进展终止 | -75 | 经过 grace 后再持续 512 步几乎没有净接近 |
| 超时 | -75 | 4096 步仍未完成 |
| 每步代价 | -0.005 | 鼓励更短路径，但不压倒连续进度奖励 |

SAC 在跨 rank 梯度平均之后统一使用 `max_grad_norm=10`；PPO 继续使用自己的
`max_grad_norm=0.5`。`run_config.json` 会完整保存 reward profile，`train_summary.csv`
同时记录奖励分项、无进展次数、正负插入比例和最终插入长度。
