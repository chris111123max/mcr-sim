...
class ExtraRolloutMetricsCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        if len(recent) > 0:
            ...
            self.logger.record(f"rollout_recent/centerline_safety_ratio_mean_w{self.window_size}", self._mean(ep["centerline_safety_ratio"] for ep in recent))
            self.logger.record(f"rollout_recent/centerline_safety_ratio_max_w{self.window_size}", self._max(ep["centerline_safety_ratio_max_episode"] for ep in recent))
            self.logger.record(f"rollout_recent/centerline_safety_margin_mean_w{self.window_size}", self._mean(ep["centerline_safety_margin"] for ep in recent))
            self.logger.record(f"rollout_recent/centerline_safety_margin_min_w{self.window_size}", self._mean(ep["centerline_safety_margin_min_episode"] for ep in recent))
        ...
        # Keep console/TensorBoard compact. In fixed-vessel runs, rollout_recent is
        # the clearest signal; task/model windows are redundant and make logs noisy.
...
def main():
    ...
    print("  algorithm=standard SAC: actor and critic use the same 46-D waypoint observation")
    print(f"  timesteps={args.timesteps}  lr={args.learning_rate:g}  batch={args.batch_size}  buffer={args.buffer_size}  ent_coef={args.ent_coef}")
