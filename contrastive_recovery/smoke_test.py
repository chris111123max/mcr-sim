...
def main():
    ...
    for step in range(24):
        ...
        replay.add_batch(obs, actions, next_obs, dones, np.random.randn(envs).astype(np.float32), np.random.rand(envs).astype(np.float32), np.clip((step + 1) / 24., 0., 1.) * np.ones(envs, dtype=np.float32))
        obs = next_obs
    ...
    metrics = agent.update(replay.sample(4, length, 6, 4))
